...
// Server-side only exports (these will throw errors if used on client)
let account: Account | null = null
let client: StoryClient | null = null
let publicClient: ReturnType<typeof createPublicClient> | null = null
let walletClient: WalletClient | null = null

// Only initialize these on server-side
if (typeof window === 'undefined' && process.env.WALLET_PRIVATE_KEY) {
    account = privateKeyToAccount(`0x${process.env.WALLET_PRIVATE_KEY}` as Address)
...
export { account, client, publicClient, walletClient }